...
/// Fills the OTP field automatically from the verification SMS, so the rider
...
///    (and different between debug and release builds). The backend would
///    have to append that hash to every OTP message for it to work at all.
...
///
/// Since the OTP SMS format isn't ours to change from here, User Consent is
/// the only one of the two that can be relied on to work.
///
...
class OtpSmsRetriever implements SmsRetriever {
  ...
  /// (a helpline, an order id) can't be mistaken for the code. This app's
  /// OTP is 4-digit; Pinput additionally drops anything whose length doesn't
  /// match the field's, so a wrong-length match is discarded rather than
  /// half-filled.
  ...
  /// later would overwrite whatever the rider had since typed by hand.
  @override
  ...
  Future<String?> getSmsCode() async {
    ...
    // the rider dismissing the consent dialog — without treating a dismissal
