...
import 'package:vivashri/call_parent/chat/api/direct_chat_controller.dart';
...
class APIs with ChangeNotifier {
  static final UserDetailController myuserController =
      Get.find<UserDetailController>();
  // final u = _myuserController.userData.value!;

  static String myid = myuserController.userData.value?.profileId ?? "";
  static String name = myuserController.userData.value?.name ?? "";
  static String email = myuserController.userData.value?.email ?? "";
...
static Stream<int> getUnreadMessageCountStream(ChatUser user) {
  ...
  static Future<bool> userExists() async {
    print("my user ====${myuserController.userData.value?.profileId ?? ""}");

    return (await firestore
            .collection('users')
            .doc(myuserController.userData.value?.profileId ?? "")
            .get())
        .exists;
  }
  ...
  static Future<void> setTypingStatus(String userId, bool status) async {
    final ref = firestore.collection(
      'chats/${getConversationID(userId)}/typing_status/',
    );

    await ref.doc(myid).set({'typingStatus': status});
  }

  // // Function to get typing status
  static Stream<bool> getTypingStatus(String userId) {
    final ref = firestore
        .collection('chats/${getConversationID(userId)}/typing_status')
        .doc(userId);

    return ref.snapshots().map((doc) {
      if (doc.exists) {
        return doc.data()?['typingStatus'] ?? false;
      } else {
        return false;
      }
    });
